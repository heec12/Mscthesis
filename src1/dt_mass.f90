...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Stole from Tucholke's code
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
if (nloop.le.1000) then
    strninert = strain_inert0
elseif (nloop.gt.1000 .and. nloop.le.2000) then
!    strninert = strain_inert0 + (strain_inert - strin_inert0)/(2001-nloop)
    strninert = strain_inert0 + (strain_inert - strain_inert0)/1000*(nloop-1000)
elseif (nloop.gt.2000 .and. mloop.le.1000) then
!    strninert = strain_inert0 + (strain_inert - strin_inert0)/(1001-mloop)
    strninert = strain_inert0 + (strain_inert - strain_inert0)/1000*mloop
else
...
!        print *, 'dt_elastic = ', dt_elastic, 'strninert = ', strninert
...
!        if (vbc.gt.0) then
!            dt_elastic = dlmin*frac*strninert/vbc
!            print *, 'vbc > 0, dt_elastic = ', dt_elastic, 'nloop = ', nloop, 'mloop = ', mloop, 'strninert = ', strninert
!        else
!            dt_elastic = dlmin*frac*strninert/(1./sec_year/100)
!            print *, 'vbc < 0, dt_elastic = ', dt_elastic, 'nloop = ', nloop, 'mloop = ', mloop, 'strninert = ', strninert
!        endif
