...
do while( time .le. time_max )
  nloop = nloop + 1
!  itest_rem1 = itest_rem1 + 1
!  itest_rem2 = itest_rem2 + 1
  mloop = mloop + 1
if( dtout_screen .ne. 0 ) then
  ...
  ! print averaged velocity for every time step
  svelleft = 0.0
  avelleft = 0.0
  svelright = 0.0
  avelright = 0.0
  ...
  do i = 1,30
      svelleft = svelleft + vel(1,i,1)
!      print *, 'i = ', i, 'vel(1,i,1) = ', vel(1,i,1)
      avelleft = svelleft*sec_year*1.e2/30
  enddo
  do i = 91,nx
      svelright = svelright + vel(1,i,1)
      avelright = svelright*sec_year*1.e2/31
  end do
  open (1,file='velocity.txt',position='append')
  !write (1,'(i10,1x,f7.3,1x,e10.3,1x,e10.3)') time/sec_year/1.e3, avelleft, avelright
  write (1,'(f7.3,1x,f7.3,1x,f7.3)') time/sec_year/1.e3, avelleft, avelright
  close (1)
